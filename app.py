...
with st.sidebar:
    ...
    if st.session_state.mode in ("classic", "engine"):
        ...
        # --- Options VS — bloc explicatif (Jalon 5.3.18) -----------------
        # Les CONTRÔLES (segmented controls VS/VU + Standard/API Plaques)
        # vivent dans la page Moteur, à côté du bouton « Segmenter en
        # flottes » — c'est là que l'AM prend ses décisions d'onboarding.
        # Ici dans la sidebar on garde juste un rappel des deux règles
        # pour que l'utilisateur puisse les consulter à tout moment.
        with st.expander("🚙 Options VS — comment ça marche ?", expanded=False):
            st.markdown(
                "Les 2 boutons en haut de la page **Moteur** s'appliquent "
                "uniquement aux véhicules classés **VS**.\n\n"
                "**1️⃣ Dénomination dans la sortie**\n"
                "- **VS** _(défaut)_ → `usage = service` dans la table "
                "Véhicules.\n"
                "- **VU** → `usage = utility`. Le drapeau fiscal "
                "**isHT** n'est pas touché : les contrats VS restent en HT.\n\n"
                "**2️⃣ Logique HT/TTC**\n"
                "- **Standard** _(défaut)_ → on présume HT pour un VS, "
                "sauf détection explicite TTC (nom de colonne, valeur "
                "de cellule, ou Genre/Carrosserie loueur qui dit VP).\n"
                "- **API Plaques fait foi** → si la détection explicite "
                "ne tranche pas, **api_plaques.genreVCGNGC** arbitre. "
                "SIV dit VP → on assume TTC et on convertit en HT. SIV "
                "dit VU/VS/VASP → on assume HT. Le `usage` Vehicle "
                "n'est pas modifié par ce mode."
            )
...
def _render_engine_fleet_controls() -> None:
    ...
    Jalon 5.3.18 — augmenté avec les **Options VS** (dénomination VS/VU
    + logique HT/TTC standard/api_plaques) qui s'appliquent uniquement
    aux véhicules classés VS. La doc détaillée des deux règles vit
    dans la sidebar (expander « 🚙 Options VS »).
    """
    current_fleet_mapping = st.session_state.get("engine_fleet_mapping")

    # ── Row 1 : segmentation flotte (inchangée) ──────────────────────────
    ...
    # ── Row 2 : Options VS (Jalon 5.3.18) ────────────────────────────────
    # Petit encart visuel cohérent avec la pill segmentation flotte au-dessus.
    _has_segmented = hasattr(st, "segmented_control")

    # Header avec popover d'aide aligné à droite — UX cohérente avec
    # le reste du moteur.
    h_left, h_pop = st.columns([3, 1])
    with h_left:
        st.markdown(
            "<div style='margin-top:0.85rem;font-weight:600;color:#1f2937;"
            "font-size:0.95rem'>🚙 Options VS</div>"
            "<div style='color:#6b7280;font-size:0.8rem;margin-bottom:0.4rem'>"
            "Ajuste comment les VS sont traités à l'export.</div>",
            unsafe_allow_html=True,
        )
    with h_pop:
    ...
    # Les 2 segmented controls côte à côte.
    sc1, sc2 = st.columns(2)
    with sc1:
        st.markdown(
            "<div style='font-size:0.8em;color:#6b7280;margin-bottom:0.2rem'>"
            "Dénomination dans la sortie</div>",
            unsafe_allow_html=True,
        )
        if _has_segmented:
    ...
    # Pills d'état — feedback immédiat sur ce qui est actif.
    _denom_pill = (
    ...
    st.markdown(
        ...
        "margin-top:0.45rem;font-size:0.8em'>"
