"""Package the engine output into a single downloadable zip (Jalon 3.0).

The UI requirement is « un seul bouton, un seul zip » with the following
layout::

    revio_import_<client>_<ts>/
    ├── onboarding_complet.xlsx      (master workbook, multi-tabs)
    ├── rapport.xlsx                  (per-cell source / anomalies — optional)
    └── vehicles/
...
ship only a single ``vehicles_tous.csv`` plus the « tous » tab in the
master Excel. No empty artifacts.

The module is schema-aware (only ``vehicle`` for now) but structured so
that ``contract``, ``driver``, ``asset`` can be plugged in Jalon suivant
without rewriting the output layout.
"""
...
def build_master_xlsx(
    *,
    ...
    For now only ``vehicle`` is populated. The structure is ready to drop
    contract / driver / asset dataframes in, each contributing:
        - 1 « tous » tab
    ...
    if vehicle_df is not None and not vehicle_df.empty:
        ...
        # Per-fleet tabs (only when segmentation is active)
...
def build_output_zip(
    *,
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        # 3. Existing per-cell rapport.xlsx — passthrough.
